...
import richard.lipa.Api_ForoHub.domain.respuesta.*;
import richard.lipa.Api_ForoHub.domain.topico.Topico;
import richard.lipa.Api_ForoHub.domain.topico.TopicoRepository;
    ...
    public ResponseEntity crearRespuesta(@RequestBody @Valid DatosRegistroRespuesta datos, UriComponentsBuilder uriComponentsBuilder){
      /*  Usuario usuario = usuarioRepository.findById(datos.autorId())
        ...
        if(usuarioOptional.isPresent() && topicoOptional.isPresent()){
            ...
            String mensaje = "⚠️ ⚠️El ID " + datos.topicoId() + " no corresponde a ningún tópico registrado.";
   ...
   @Transactional
    ...
    public  ResponseEntity actualizar(@PathVariable Long id, @RequestBody  DatosActualizarRespuesta datos){
       ...
       System.out.println("respuest Optional: "+ respuestaOptional);
        ...
        String mensaje = "⚠️ ⚠️El ID " + id + " no corresponde a ningún tópico registrado.";
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(mensaje);
